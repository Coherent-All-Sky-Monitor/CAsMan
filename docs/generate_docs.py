...
class MarkdownGenerator:
    ...
    def generate_module_docs(self, module_info: ModuleInfo) -> str:
        ...
        # Module header
        lines.extend(
            [f"# {module_info.name.title()} Module", "", module_info.docstring, ""]
        )
        ...
        if module_info.constants:
            ...
            for name, value in module_info.constants.items():
                lines.extend([f"### {name}", "", f"**Value:** `{repr(value)}`", ""])
    ...
    def _generate_function_docs(self, func_info: FunctionInfo) -> List[str]:
        ...
        lines.extend([f"**Signature:** `{signature}`", ""])
    ...
    def _generate_class_docs(self, class_info: ClassInfo) -> List[str]:
        ...
        lines.extend([f"### {class_info.name}", ""])
        ...
        lines.extend([f"**Class:** `{class_signature}`", "", class_info.docstring, ""])
...
class DocumentationGenerator:
    ...
    def _generate_package_docs(self, package_name: str) -> None:
        ...
        if not package_path.is_dir():
            ...
            if package_path.with_suffix(".py").exists():
                module_info = self.analyzer.analyze_file(
    ...
    def _generate_package_overview(
        ...
        if init_module and init_module.docstring != "No module docstring available.":
            lines.extend(["## Overview", "", init_module.docstring, ""])
        ...
        if non_init_modules:
            ...
            for module in non_init_modules:
                lines.extend([f"### {module.name}", "", module.docstring, ""])
                ...
                if module.functions:
                    ...
                    for func_name, func_info in module.functions.items():
                        lines.append(
                            f"- `{func_name}()` - {func_info.docstring.split('.')[0]}"
                        )
                    lines.append("")
                ...
                if module.classes:
                    ...
                    for class_name, class_info in module.classes.items():
                        lines.append(
                            f"- `{class_name}` - {class_info.docstring.split('.')[0]}"
                        )
                    lines.append("")
        ...
        for module in modules:
            if module.functions or module.classes:
                lines.extend([f"## {module.name.title()} Module Details", ""])
    ...
    def _generate_cli_docs(self) -> None:
        ...
        for cmd, info in commands.items():
            ...
            if isinstance(description, str):
                lines.extend([f"### casman {cmd}", "", description, ""])
...
def main() -> None:
    ...
    try:
        ...
        print(
            "\nAuto-generated documentation created in",
            PROJECT_ROOT / "docs" / "auto-generated",
        )
        print("Files created:")
