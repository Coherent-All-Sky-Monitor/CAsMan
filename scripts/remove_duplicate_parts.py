        ...
        for rowid, scan_time, connected_to in rows:
            if rowid not in keep:
                ...
                with open(log_path, 'a', encoding='utf-8') as logf:
                    logf.write(f"[{now}] Removed duplicate for part_number {part_number} (scan_time={scan_time}, connected_to={connected_to})\n")
    conn.commit()
    conn.close()
    print("Duplicate removal complete. See logs/connection_modification_logs for details.")
