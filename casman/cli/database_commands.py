...
def cmd_database_clear(parser: argparse.ArgumentParser, remaining_args: list) -> None:
    ...
    def clear_parts_db_local(db_dir: str) -> None:
        ...
        confirm1 = input("Are you sure you want to clear the parts database? (yes/no): ").strip().lower()
        if confirm1 != "yes":
            print("Aborted.")
            return
        confirm2 = input("This action is IRREVERSIBLE. Type 'yes' again to confirm: ").strip().lower()
        if confirm2 != "yes":
            print("Aborted.")
            return
        try:
    ...
    def clear_assembled_db_local() -> None:
        ...
        if db_path is None:
            ...
            sys.exit(1)
        print_stop_sign()
        print(f"WARNING: This will DELETE ALL records from the assembled_casm database at: {db_path}")
        confirm1 = input("Are you sure you want to clear the assembled_casm database? (yes/no): ").strip().lower()
        if confirm1 != "yes":
            print("Aborted.")
            return
        confirm2 = input("This action is IRREVERSIBLE. Type 'yes' again to confirm: ").strip().lower()
                ...
                if table_width <= term_width:
                    ...
                    for row in rows:
                        for i, val in enumerate(row):
                            print(f"  {abbr_names[i]:<{max(8, len(abbr_names[i]))}} : {val if val is not None else 'NULL'}")
                        print(sep_line)
            else:
