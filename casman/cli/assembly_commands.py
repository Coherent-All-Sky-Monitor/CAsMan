...
def cmd_scan() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='CAsMan Scanning and Assembly')
...
def cmd_assemble() -> None:
    ...
    parser.add_argument('--part1', required=True,
                        help='First part number')
    parser.add_argument('--part1-type', required=True,
                        help='First part type')
    parser.add_argument('--part2', required=True,
                        help='Second part number or SNAP connection')
    parser.add_argument('--part2-type', required=True,
                        help='Second part type')
    parser.add_argument('--polarization', required=True,
                        help='Polarization for both parts')
    ...
    if args.action == 'connect':
        ...
        if not part1_exists:
            print(
                f"Error: {args.part1_type} part {args.part1} not found in database")
            return
        ...
        success = record_assembly_connection(
            ...
            connected_polarization=args.polarization if args.part2_type != 'FENGINE' else 'N/A',
            connected_scan_time=connected_scan_time)

        if success:
            print(
                f"Successfully recorded connection: {args.part1} ---> {args.part2}")
        else:
            print(
                f"Error recording connection: {args.part1} ---> {args.part2}")
