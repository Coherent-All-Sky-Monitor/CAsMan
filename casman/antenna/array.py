...
class AntennaArray:
    ...
    def from_database(
        ...
        # Download/sync database if requested
        if sync_first:
            if db_dir is None:
                sync_db_dir = str(db_path.parent)
            else:
                sync_db_dir = db_dir
            ...
            if result['success']:
                ...
                if not db_path.exists():
                    raise FileNotFoundError(
                        f"Database download failed and no local copy exists at {db_path}"
                    )
                print(f"Continuing with local database at {db_path}")
...
__all__ = ["AntennaPosition", "AntennaArray", "sync_database"]
