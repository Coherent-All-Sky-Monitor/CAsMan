...
def get_all_parts():
    ...
    c = conn.cursor()
    # Fetch all unique part numbers from both part_number and connected_to columns
    c.execute(
...
def get_all_chains(selected_part=None):
    """
    Fetch all connection chains from the assembled_casm.db database.

    Parameters
    ----------
    selected_part : str or None, optional
    ...
    tuple
        (all_chains, connections):
            all_chains : list of list of str
                List of all connection chains (each chain is a list of part numbers).
            connections : dict
    ...
    records = c.fetchall()
    # Map parts to their connections
    connections = {}
    ...
    conn.close()
    # Build all chains if no specific part is selected
    all_chains = []
    visited = set()
...
def build_chain(start_part, connections, visited):
    ...
    ----------
    start_part : str
        The part number to start the chain from.
    connections : dict
        Mapping from part_number to (connected_to, scan_time, connected_scan_time).
    visited : set
        Set of part numbers already visited in this traversal.

    Returns
    -------
    list of str
        The chain of connected part numbers starting from start_part.
    """
    chain = []
    queue = [start_part]
    while queue:
...
def format_display_data(part, connections):
    """
    ...
    ----------
    part : str
        The part number to format display data for.
    connections : dict
        Mapping from part_number to (connected_to, scan_time, connected_scan_time).
    ...
    -------
    str
        HTML-formatted string for display in the web interface.
    """
    # Get scan times for the part and its connections
    _, scan_time, connected_scan_time = connections.get(part, (None, None, None))
    display_lines = [
        f"<span class='national-park'>{part}</span><br>"
    ]  # Added two line breaks for spacing
    ant, lna, bac = "ANTENNA", "LNA", "BACBOARD"
    # Display scan times for each part type
    if "ANT" in part:
        ...
        # Retrieve the ANTENNA's connected-to scan time from the ANTENNA entry
        ant_entry = next(
        ...
        # Retrieve the LNA's connected-to scan time from the LNA entry
        lna_entry = next(
            (p for p, (c, _, cs) in connections.items() if c == part), None
        )
...
def get_last_update():
    ...
    # Fetch the maximum timestamp from both scan_time and connected_scan_time
    c.execute(
