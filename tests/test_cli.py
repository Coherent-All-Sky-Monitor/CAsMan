...
class TestCLI:
    ...
    def test_main_version(self) -> None:
        ...
        with patch("sys.argv", ["casman", "--version"]):
            with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
                ...
                output = mock_stdout.getvalue()
                assert any(word in output.lower() for word in ["version", "casman"])
    ...
    def test_cmd_parts_add(
        self, mock_init_db: Mock, mock_add_interactive: Mock
    ) -> None:
    ...
    def test_cmd_parts_list(self, mock_init_db: Mock, mock_get_parts: Mock) -> None:
    ...
    def test_cmd_parts_interactive(
        self, mock_init_db: Mock, mock_interactive: Mock
    ) -> None:
    ...
    def test_cmd_scan_stats(self, mock_init_db: Mock, mock_get_stats: Mock) -> None:
    ...
    def test_cmd_barcode_printpages(self, mock_generate_printpages: Mock) -> None:
