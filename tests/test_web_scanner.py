    ...
    def test_get_existing_connections_only_latest_status(self, temp_db):
        ...
        cursor.execute('''
            ...
            (part_number, part_type, polarization, scan_time, 
             ...
             connected_scan_time, connection_status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', ('ANT00001P1', 'ANTENNA', '1', '2025-01-01 00:10:00',
              'LNA00001P1', 'LNA', '1', '2025-01-01 00:10:01', 'disconnected'))
...
class TestScannerRoutes:
    ...
    def test_get_connections_missing_part(self, client):
        ...
        response = client.post('/scanner/api/get-connections',
                              json={})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data['success'] is False
    ...
    def test_check_snap_ports_missing_params(self, client):
        ...
        response = client.post('/scanner/api/check-snap-ports',
                              json={})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data['success'] is False
    ...
    def test_format_snap_missing_params(self, client):
        ...
        response = client.post('/scanner/api/format-snap',
                              json={'chassis': 1})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data['success'] is False
    ...
    def test_format_snap_success_disconnect_mode(self, client, temp_db):
        ...
        cursor.execute('''
            ...
            (part_number, part_type, polarization, scan_time, 
             ...
             connected_scan_time, connection_status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', ('BAC00001P1', 'BACBOARD', '1', '2025-01-01 00:00:00',
              'SNAP1A00', 'SNAP', 'N/A', '2025-01-01 00:00:01', 'connected'))
        conn.commit()
        conn.close()
    ...
    def test_record_connection_invalid_sequence(self, client):
        ...
        response = client.post('/scanner/api/record-connection',
                              json={
    ...
    def test_record_connection_success(self, client, temp_db):
        ...
        with patch('casman.web.scanner.get_database_path', return_value=temp_db):
            with patch('casman.web.scanner.record_assembly_connection', return_value=True):
                response = client.post('/scanner/api/record-connection',
                                      json={
                                          'part_number': 'ANT00001P1',
                                          'part_type': 'ANTENNA',
    ...
    def test_record_connection_failure(self, client, temp_db):
        ...
        with patch('casman.web.scanner.get_database_path', return_value=temp_db):
            with patch('casman.web.scanner.record_assembly_connection', return_value=False):
                response = client.post('/scanner/api/record-connection',
                                      json={
                                          'part_number': 'ANT00001P1',
                                          'part_type': 'ANTENNA',
    ...
    def test_record_disconnection_success(self, client):
        ...
        with patch('casman.web.scanner.record_assembly_disconnection', return_value=True):
            response = client.post('/scanner/api/record-disconnection',
                                  json={
                                      'part_number': 'ANT00001P1',
                                      'part_type': 'ANTENNA',
                                      'polarization': '1',
    ...
    def test_record_disconnection_failure(self, client):
        ...
        with patch('casman.web.scanner.record_assembly_disconnection', return_value=False):
            response = client.post('/scanner/api/record-disconnection',
                                  json={
                                      'part_number': 'ANT00001P1',
                                      'part_type': 'ANTENNA',
    ...
    def test_add_parts_missing_params(self, client):
        ...
        response = client.post('/scanner/api/add-parts',
                              json={'count': 5})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data['success'] is False
    ...
    def test_add_parts_success(self, client):
        ...
        with patch('casman.parts.generation.generate_part_numbers', 
    ...
    def test_part_history_missing_part_number(self, client):
        ...
        response = client.post('/scanner/api/part-history',
                              json={})
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data['success'] is False
    ...
    def test_part_history_success(self, client, temp_db):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', ('ANT00001P1', 'ANTENNA', 'LNA00001P1', 'LNA', 'connected', '2025-01-01 00:00:00'))
        conn.commit()
        conn.close()
